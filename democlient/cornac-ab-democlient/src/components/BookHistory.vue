    ...
    >
      <swiper-slide v-for="book in books">
        <div class="aspect-h-2 aspect-w-1 w-36 mx-2 overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
          <img loading="lazy" :src="book.attributes.image" :alt="book.attributes.title" class="w-full object-cover object-center group-hover:opacity-75" />
          <div class="swiper-lazy-preloader dark:swiper-lazy-preloader-white"></div>
        </div>
        <h3 class="mt-4 text-sm text-gray-700 dark:text-white">{{ book.attributes.authors }}</h3>
        <p class="mt-1 truncate text-lg font-medium text-gray-900 dark:text-white">{{ book.attributes.original_title }}</p>
        <div class="flex">
