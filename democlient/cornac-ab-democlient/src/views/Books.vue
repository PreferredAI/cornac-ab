      ...
      <div class="mx-auto py-8 sm:py-8 lg:py-8">
        <div>
          <div class="mx-auto max-w-2xl px-4 py-4 sm:px-6 lg:max-w-7xl lg:px-8">
            <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl dark:text-white">Books previously rated</h2>
          ...
          <div class="mx-auto max-w-2xl px-4 py-4 sm:px-6 lg:max-w-7xl lg:px-8">
            ...
            <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl dark:text-white">Books Recommended for you</h2>
            <p class="mt-4 text-gray-500 dark:text-gray-300">These books are specially curated and recommended. Click on any of these books to view more about them.</p>
            <div v-if="loading" class="mt-8">
              <div class="flex justify-center dark:text-white">
                Loading...
              </div>
            ...
            <div v-else class="grid grid-cols-1 gap-x-24 gap-y-12 mt-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-16">
              <button v-for="item in data.items" :key="item.itemId" v-on:click="" @click="openModal(item)" class="group">
                ...
                <h3 class="mt-4 text-sm text-gray-700 dark:text-gray-300">{{ item.attributes.authors }}</h3>
                <p class="mt-1 text-lg font-medium text-gray-900 dark:text-gray-100">{{ item.attributes.original_title }}</p>
              </button>
